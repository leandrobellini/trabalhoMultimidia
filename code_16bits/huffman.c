...
void huffmanComprime(FILE *,char *, wav_hdr *, int numeroAmostras);
...
void quick(No **, int, int);


//funcao chave para compressao
void huffmanComprime(FILE *arqOrigem,char *nomeDestino, wav_hdr *cabecalho, int numeroAmostras){ 
    int i;
    ...
    for(i = 0; i < ASCII; i++){
        ...
        while( !feof(arqOrigem)){
            fread(&sample1, sizeof(unsigned short), 1, arqOrigem);
            tabela[sample1]->num++;

        } 

        /*for(i = 0; i < ASCII; i++){
            printf("Amostra %u, frequencia: %d\n", tabela[i]->c, tabela[i]->num);
        }*/
        ...
        quick(novos, 0, validos-1);

        i = validos;

        for(i = 0; i < validos; i++){
            printf("Amostra %u, frequencia: %d\n", novos[i]->c, novos[i]->num);
        }
        ...
        while(i != 1){
            ...
            aux->c = '\0'; //nó de ligação --> Apenas os nós folhas guardam realmente informações
        ...
        return;
                ...
                //vou para o inicio do arquivo --> faço toda leitura novamente e para caracter, gravo sequencia de bits no arquivo...
        rewind(arqOrigem);

        //pulo cabeçalho
        fseek(arqOrigem, sizeof(wav_hdr),SEEK_SET);

        arqDestino = fopen(nomeDestino,"wb");
        ...
        */
        ...
        fwrite(&validos,1,sizeof(int),arqDestino);
        ...
        printf("\n\t-> Arquivo comprimido com sucesso: %s\n\n",nomeDestino);
}

//função chave da descompressao do arquivo
void huffmanDescomprime(char *nomeOrigem, char *nomeDestino){
    /**
                                        DEFINIÇÃO DO CABEÇALHO
    ...
    printf("\n\t-> Reconstruindo Tabela de frequencia apartir do arquivo %s...",nomeOrigem);
    ...
    fwrite(&header, sizeof(wav_hdr), 1, arqDestino);
            ...
            //tamanho da tabela de frequência --> primeiro byte do arquivo...
    int tamanhoTabela;
    fread(&tamanhoTabela,1,sizeof(int),arqOrigem);
    ...
    printf("\n\t-> Reconstruindo a Arvore de huffman apartir do arquivo %s...",nomeOrigem);
    ...
    printf("\n\t-> Arquivo descomprimido com sucesso: %s\n\n",nomeDestino);
...
void emOrdem(No *raiz){
    ...
    emOrdem(raiz->esq);
    if(raiz->c != '\0')
        printf("\n%c[%d] => %s",raiz->c,raiz->c,raiz->codigoBinario);
    emOrdem(raiz->dir);
}
...
void quick(No **tabela, int esq, int dir){
    int pivo = esq, i,j;         //Declaração das variavés e inicialização do pivo com o primeiro algarismo da sequencia
    No ch;
    for(i=esq+1;i<=dir;i++){        //Percorre todos os espaços do vetor
        j = i;                      //atribuição de valor
        if(tabela[j]->num < tabela[pivo]->num){     //verifica se o vetor da posição pivo é maior que de outra posição
         ch = *(tabela[j]);               //ch recebe o valor que é menor
        while(j > pivo){           //repete enquanto o j que é a posição do algarismo menor que o pivo ficar na posição 0
            *(tabela[j]) = *(tabela[j-1]);      //reorganiza a posição de vetores
            j--;                    //decremento para a organização
        }
        *(tabela[j]) = ch;               // atribuição da variavel menor que o pivo na posição inicial
        pivo++;                    // aumenta a posição do pivo em uma unidade
        }  
    }
    if(pivo-1 >= esq){              // verifica se o valor do pivo é maior que o final do vetor.
        quick(tabela,esq,pivo-1);      //final da execursão da função
    }
    if(pivo+1 <= dir){              //verifica se o valor do pivo é menor, indicando que ainda estar dentro das limitações do vetor
        quick(tabela,pivo+1,dir);      //chama a função para eecutar novamente
    }
}
